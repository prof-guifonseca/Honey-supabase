...
(function () {
  ...
  function setupMemoriesPanel(container) {
    ...
    // Interface de login usando Google OAuth. A autenticação agora é feita
    // via "Sign in with Google", permitindo acesso somente aos e‑mails
    // autorizados (configurados no backend). A antiga interface de usuário e
    // senha foi removida para simplificar o fluxo de login.
    loginSection.innerHTML = `
      <h3>🔐 Diário compartilhado na nuvem</h3>
      <p class="subtitle">Entre com sua conta Google para salvar e ler memórias conjuntas.</p>
      <div id="google-signin-container" style="margin-bottom: 8px;"></div>
    ...
    function showApp() {
      ...
      // Atualiza progresso e diário ao entrar. Isto garante que os rótulos
      // de progresso dos dias e o diário reflitam memórias mais recentes.
      if (typeof window !== 'undefined') {
        if (typeof window.loadProgress === 'function') {
          window.loadProgress();
        }
        if (typeof window.updateDiary === 'function') {
          window.updateDiary();
        }
      }
    }
    ...
    function renderMemories(memories) {
      ...
      memories.forEach((mem) => {
        ...
        // Botão de exclusão. Visível apenas quando o usuário está logado. A
        // exclusão é protegida no backend para que somente o autor possa
        // deletar sua própria memória; caso contrário, uma mensagem de erro
        // será retornada. O botão é posicionado ao final do cartão.
        ...
        const res = await fetch(deleteUrl, {
          ...
          credentials: 'include',
            ...
            if (!res.ok) {
              alert(data.error || 'Erro ao excluir memória.');
              return;
            }
            // Após excluir, recarrega a lista e painéis de memórias de dias
            loadMemories();
            if (typeof window !== 'undefined' && typeof window.refreshAllDayMemories === 'function') {
              window.refreshAllDayMemories();
            }
            // Atualiza capa e diário público
            if (typeof window !== 'undefined' && typeof window.loadCoverMemories === 'function') {
              window.loadCoverMemories();
            }
            if (typeof window !== 'undefined' && typeof window.updateDiary === 'function') {
              window.updateDiary();
            }
          } catch (err) {
