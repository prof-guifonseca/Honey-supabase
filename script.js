...
document.addEventListener('DOMContentLoaded', async () => {
  // Initialise Supabase client.  The URL and anon key are read from
  // meta tags injected into index.html.  We dynamically import the
  // supabase-js library from a CDN to avoid bundling it locally.  Once
  // imported we attach the client to the global `window` object so
  // helper functions can reference it.
  try {
    ...
    const supabaseAnonKey = supabaseAnonMeta ? supabaseAnonMeta.getAttribute('content') : '';
...
function initScheduleItemsForDay(panel, dayId) {
  ...
  listItems.forEach((li, index) => {
    ...
    doneBtn.addEventListener('click', async (ev) => {
      ...
      try {
        // Update progress using Supabase function.  We compute the
        // item identifier as `${dayId}-${index}` to mirror the server
        // semantics.  The function returns the updated progress map on
        // success.
        if (!window.supabase) throw new Error('Supabase client não inicializado');
        const { data, error } = await window.supabase.functions.invoke('progress', {
          method: 'PUT',
          headers: { 'Content-Type': 'application/json' },
          body: { done: newState },
          query: { item_id: `${dayId}-${index}` },
        });
        if (error) {
          if (error.status === 401) {
          ...
          throw new Error(error.message || 'Falha ao atualizar progresso');
        }
...
async function checkAuthStatus() {
  // Consult the current session via Supabase.  If a user is logged in,
  // `supabase.auth.getUser()` returns their profile.  We map the
  // Supabase user to the legacy session format expected by the UI.
  try {
    if (!window.supabase) {
    ...
    // Construct a minimal session object compatible with the old code
    const sessionUser = {
      username: user.email,
...
async function loadProgress() {
  try {
    if (window.supabase) {
      const { data, error } = await window.supabase.functions.invoke('progress', { method: 'GET' });
      if (!error) {
        window.progress = data || {};
      } else {
        window.progress = {};
...
async function updateDiary() {
  ...
  // Fetch public memories from Supabase.  We query the memories
  // table directly via the JS client, filtering by visibility = 'public'.
  let memories = [];
  try {
    if (window.supabase) {
...
function registerGoogleButton(target, feedbackEl) {
  if (!target) return;
  // Renderiza um botão personalizado que aciona o fluxo de login do
  // Supabase via OAuth (Google).  Quando clicado, chamamos
  // supabase.auth.signInWithOAuth.  Se ocorrer um erro, exibimos
  // uma mensagem no feedbackEl (se fornecido).
  target.innerHTML = '';
  const btn = document.createElement('button');
  btn.type = 'button';
  btn.className = 'supabase-google-btn';
  ...
  btn.addEventListener('click', async () => {
    if (!window.supabase) {
      if (feedbackEl) {
        feedbackEl.textContent = 'Supabase não inicializado';
    ...
    try {
      ...
      if (feedbackEl) {
        feedbackEl.textContent = 'Erro ao iniciar login Google.';
        feedbackEl.classList.add('error');
      }
    }
  });
...
async function handleGoogleCredential(resp, feedbackEl) {
  // Migrated login using Supabase Auth.  If a credential is provided
  // by the Google Identity Services (resp.credential), we attempt
  // signInWithIdToken which accepts a Google ID token.  Otherwise
  // we fall back to the OAuth redirect flow.  On success, we
  // refresh the authentication state using checkAuthStatus.
  try {
    if (!window.supabase) {
      if (feedbackEl) {
        feedbackEl.textContent = 'Supabase não inicializado.';
    ...
    if (idToken) {
      const { error } = await window.supabase.auth.signInWithIdToken({ provider: 'google', token: idToken });
      if (error) {
        if (feedbackEl) {
          feedbackEl.textContent = error.message || 'Login não autorizado.';
          feedbackEl.classList.add('error');
        }
        setAuthState(false, null);
        return;
    ...
    // Reevaluate auth status after login
    await checkAuthStatus();
  } catch (err) {
    if (feedbackEl) {
